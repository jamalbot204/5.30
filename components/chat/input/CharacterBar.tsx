...
const CharacterBar: React.FC<CharacterBarProps> = memo(({ 
    ...
    return (
        <div 
            ref={containerRef} 
            className="p-3 border-b border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-black/10" 
            ...
            <div className="flex items-center flex-nowrap w-full">
                ...
                {showSwitchButton && onSwitchToolbar && (
                    <Button
                        ...
                        variant="ghost"
                        size="none"
                        className="p-1.5 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white rounded-xl hover:bg-gray-200 dark:hover:bg-white/10 transition-colors flex-shrink-0 me-3"
                    ...
                    <div className="w-max flex items-center gap-2 px-1">
                        {characters.map((char) => (
                            <Button 
                                ...
                                className={`px-3 py-1.5 text-xs font-medium bg-fuchsia-600 dark:bg-fuchsia-500 text-white rounded-xl disabled:opacity-50 transition duration-200 ease-out whitespace-nowrap ${isReorderingActive ? 'cursor-grab hover:ring-2 hover:ring-fuchsia-400' : 'disabled:cursor-not-allowed'} ${draggedCharRef.current?.id === char.id ? 'opacity-50 ring-2 ring-fuchsia-500' : ''}`}
                            >
                                {char.name}
                            </Button>
                        ))}
