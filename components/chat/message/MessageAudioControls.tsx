...
const MessageAudioControls: React.FC<MessageAudioControlsProps> = memo(({ 
    ...
    const renderPlayButtonForSegment = (partIndexInput?: number) => {
        ...
        let IconComponent = SpeakerWaveIcon, iconClassName = segmentState.isAudioReadyToPlayFromCacheForSegment ? 'text-emerald-500 dark:text-emerald-400' : 'text-gray-500 dark:text-gray-400', title = segmentState.isAudioReadyToPlayFromCacheForSegment ? t.playCached : t.playMessage, isDisabled = false, isPulsing = false;
        ...
        if (isStreaming) { IconComponent = SpeakerWaveIcon; isPulsing = true; isDisabled = true; title = isMainContextButton ? t.loadingAudio : `${t.loadingPart} ${partIndexInput! + 1}...`; iconClassName = 'text-emerald-600 dark:text-emerald-400';
        } else if (isThisTheMainButtonOverallFetching) { IconComponent = XCircleIcon; iconClassName = 'text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300'; title = `${t.cancelFetching} ${textSegmentsForTts.length}`; isPulsing = true;
        } else if (isThisSegmentIndividuallyFetching && !isMainContextButton) { IconComponent = XCircleIcon; iconClassName = 'text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300'; title = `${t.cancelFetching} Part ${partIndexInput! + 1}`; isPulsing = true;
        } else if (segmentState.isAudioPlayingForThisSegment) { IconComponent = PauseIcon; iconClassName = 'text-amber-500 dark:text-amber-400'; title = isMainContextButton ? t.pause : `${t.pause} Part ${partIndexInput! + 1}`;
        } else if (segmentState.isAudioLoadingForPlayer) { IconComponent = SpeakerWaveIcon; isPulsing = true; isDisabled = true; title = isMainContextButton ? t.loadingAudio : `${t.loadingPart} ${partIndexInput! + 1}...`; iconClassName = 'text-emerald-600 dark:text-emerald-400';
        } else if (segmentState.hasAudioErrorForThisSegment) { IconComponent = SpeakerXMarkIcon; iconClassName = 'text-red-500 dark:text-red-400'; title = `${isMainContextButton ? "" : `Part ${partIndexInput! + 1}: `}${t.audioError}: ${segmentState.audioErrorMessage || 'Unknown error'}.`; }
        ...
        return (
            <Button 
                ...
                variant="ghost"
                size="sm"
                className={`p-1.5 bg-gray-100 dark:bg-black/20 focus:ring-2 ring-emerald-500 hover:bg-gray-200 dark:hover:bg-white/10 ${iconClassName}`} 
