...
const DropdownMenuItem: React.FC<{
    ...
    <Button 
        ...
        icon={<Icon className={`w-5 h-5 ${disabled ? 'text-gray-500' : (iconClassName || '')}`} />}
    />
));
    ...
    return (
        <>
            <Button 
                ...
                variant="ghost"
                size="sm"
                className="p-1.5 bg-gray-100 dark:bg-black/20 focus:ring-2 ring-emerald-500 hover:bg-gray-200 dark:hover:bg-white/10"
                icon={<EllipsisVerticalIcon className="w-4 h-4" />}
            />
            {isOptionsMenuOpen && createPortal(
                <div 
                    ref={dropdownRef} 
                    className="bg-white dark:bg-zinc-900 w-auto max-w-[90vw] rounded-xl shadow-xl p-1 flex space-x-1 focus:outline-none border border-gray-200 dark:border-white/10"
                    style={menuStyle}
                    role="menu" 
                ...
                >
                    {isSelectionModeActive && lastSelectedId && lastSelectedId !== message.id && (
                        <DropdownMenuItem onClick={handleSelectRange} icon={CheckCircleIcon} label="Select Range to Here" iconClassName="text-green-600 dark:text-green-400" hoverGlowClassName="hover:bg-green-50 dark:hover:bg-green-500/10" />
                    )}
                    
                    {!isSelectionModeActive && (
                        <>
                            {(showReadModeButton) && (<DropdownMenuItem onClick={handleReadModeClick} icon={BookOpenIcon} label={t.readMode} hoverGlowClassName="hover:bg-gray-100 dark:hover:bg-white/10"/>)}
                            {(showExportPdfButton) && (<DropdownMenuItem onClick={() => { handleExportMessagePdf(message.id, exportTargetId); setIsOptionsMenuOpen(false); }} icon={PdfIcon} label={t.exportToPdf} iconClassName="text-red-600 dark:text-red-500" hoverGlowClassName="hover:bg-gray-100 dark:hover:bg-white/10" disabled={isAnyAudioOperationActiveForMessage} />)}
                            <DropdownMenuItem onClick={handleCopyMessageClick} icon={ClipboardDocumentListIcon} label={t.copyText} hoverGlowClassName="hover:bg-gray-100 dark:hover:bg-white/10"/>
                            {(isUser || isModel) && <DropdownMenuItem onClick={handleInjectPairClick} icon={ChatBubblePlusIcon} label={t.insertPairAfter} disabled={isAnyAudioOperationActiveForMessage} hoverGlowClassName="hover:bg-gray-100 dark:hover:bg-white/10"/>}
                            {message.content.trim() && !isError && allTtsPartsCached && (<DropdownMenuItem onClick={() => triggerAudioDownloadModal(message.id)} icon={ArrowDownTrayIcon} label={t.downloadAudio} disabled={isAnyAudioOperationActiveForMessage} hoverGlowClassName="hover:bg-gray-100 dark:hover:bg-white/10"/>)}
                            {!isError && (isUser || isModel) && (<DropdownMenuItem onClick={handleEditClick} icon={PencilIcon} label={t.edit} disabled={isAnyAudioOperationActiveForMessage} hoverGlowClassName="hover:bg-indigo-50 dark:hover:bg-indigo-500/10"/>)}
                            {!isError && isModel && !message.characterName && (<DropdownMenuItem onClick={() => { handleRegenerateAIMessage(message.id); setIsOptionsMenuOpen(false); }} icon={ArrowPathIcon} label={t.regenerateJustThis} disabled={isAnyAudioOperationActiveForMessage} hoverGlowClassName="hover:bg-indigo-50 dark:hover:bg-indigo-500/10"/>)}
                            {isUser && canRegenerateFollowingAI && !message.characterName && (<DropdownMenuItem onClick={() => { handleRegenerateResponseForUserMessage(message.id); setIsOptionsMenuOpen(false); }} icon={ArrowPathIcon} label={t.regenerateJustThis} disabled={isAnyAudioOperationActiveForMessage} hoverGlowClassName="hover:bg-indigo-50 dark:hover:bg-indigo-500/10"/>)}
                            <DropdownMenuItem onClick={() => { deleteSingleMessage(message.id); setIsOptionsMenuOpen(false); }} icon={XCircleIcon} label={t.deleteThisMessage} className="text-red-600 dark:text-red-400" hoverGlowClassName="hover:bg-red-50 dark:hover:bg-red-500/10" disabled={isAnyAudioOperationActiveForMessage}/>
                            <DropdownMenuItem onClick={() => { 
                                ...
                                setIsOptionsMenuOpen(false); 
                            }} icon={TrashIcon} label={t.deleteMessageHistory} className="text-red-600 dark:text-red-400" hoverGlowClassName="hover:bg-red-50 dark:hover:bg-red-500/10" disabled={isAnyAudioOperationActiveForMessage}/>
                        </>
                    )}
                </div>,
                document.body
