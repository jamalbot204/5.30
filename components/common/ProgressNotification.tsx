  ...
  return (
    ...
    >
      {progressItems.map(item => {
        ...
        if (isSuccess) borderColor = 'border-emerald-500/50';
        ...
        return (
          <div
            key={item.id}
            role="alert"
            className={`bg-white dark:bg-zinc-900 border p-4 rounded-lg shadow-2xl flex flex-col transition duration-300 ease-in-out animate-fade-in-right ${borderColor}`}
          >
            <div className="flex justify-between items-start">
              <div className="flex-grow">
                <div className="flex items-center">
                  {isSuccess && <CheckCircleIcon className="w-5 h-5 mr-2 text-emerald-500 flex-shrink-0" />}
                  {isError && <XCircleIcon className="w-5 h-5 mr-2 text-red-500 flex-shrink-0" />}
                  {isRunning && (
                    <svg className="animate-spin h-5 w-5 mr-2 text-emerald-500 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              ...
              {isRunning && item.onCancel && (
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  className="p-1.5 text-xs text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white rounded-md transition-colors h-auto"
            ...
            {isRunning && (
              <div className="mt-3">
                <div className="w-full bg-gray-200 dark:bg-black/30 rounded-full h-1.5">
                  <div
                    className="bg-emerald-500 h-1.5 rounded-full transition duration-300 ease-linear"
