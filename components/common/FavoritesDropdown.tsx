...
const FavoritesDropdown: React.FC<FavoritesDropdownProps> = memo(({
  ...
  return createPortal(
    <div
      ref={dropdownRef}
      style={style}
      className="w-72 bg-white dark:bg-zinc-900 rounded-lg shadow-2xl p-3 flex flex-col max-h-96 border border-gray-200 dark:border-white/10"
      role="menu"
      aria-orientation="vertical"
    >
      <div className="flex items-center mb-2 px-1 pb-2 border-b border-gray-200 dark:border-white/5">
        <StarIcon filled className="w-4 h-4 mr-2 text-yellow-500" />
        <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-200">Favorite Messages</h3>
      </div>
      <div className="flex-grow overflow-y-auto pr-1 -mr-1 space-y-1 custom-scrollbar">
        {favoritedMessages.length === 0 ? (
          <p className="text-xs text-gray-500 dark:text-gray-400 italic text-center py-4">No favorite messages yet.</p>
        ) : (
            favoritedMessages.map(message => {
              ...
              return (
                <div
                  key={message.id}
                  className="group flex items-center justify-between rounded-md transition-colors hover:bg-gray-100 dark:hover:bg-white/10 p-1.5"
                  role="menuitem"
                >
                  <Button
                    onClick={() => onJumpToMessage(message.id)}
                    variant="ghost"
                    className="flex-grow text-left text-xs text-gray-700 dark:text-gray-300 focus:outline-none p-0 h-auto w-auto bg-transparent hover:bg-transparent"
                    title={content}
                  >
                    <div className="line-clamp-2">
                      <span className="font-bold text-emerald-600 dark:text-emerald-400 mr-1">{message.role === 'user' ? 'You:' : (message.characterName || 'AI')}:</span>
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="none"
                    className="p-1 text-gray-400 hover:text-red-500 rounded-full opacity-0 group-hover:opacity-100 transition-opacity focus:opacity-100 flex-shrink-0 ml-2 h-auto w-auto"
