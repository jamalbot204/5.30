...
const RefreshAttachmentButton: React.FC<RefreshAttachmentButtonProps> = memo(({
  ...
  let iconColor = 'text-emerald-500 hover:text-emerald-400';
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="none"
      className={`p-1 bg-black/40 rounded-full transition h-auto w-auto
                  ${disabled || isLoading ? 'cursor-not-allowed opacity-70' : 'hover:bg-black/60'}
                  ${iconColor}
                `}
