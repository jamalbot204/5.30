    ...
    >
      ...
      >
        <div className="flex justify-between items-center mb-6">
          <h2 id="github-import-modal-title" className="text-lg font-semibold text-teal-900 dark:text-gray-100 flex items-center">
            <GitHubIcon className="w-5 h-5 mr-3 text-teal-600 dark:text-gray-100" />
        ...
        {step === 1 ? (
          <form onSubmit={handleNext}>
              {/* Input Card - Slate */}
              <div className="relative p-4 mb-6 rounded-r-xl rounded-l-md border border-slate-500/20 dark:border-white/10 border-l-4 border-l-slate-500 bg-gradient-to-r from-slate-50/50 dark:from-slate-500/5 to-transparent backdrop-blur-sm shadow-sm dark:shadow-none">
                  ...
                  <div className="relative">
                      <Input
                          ...
                          className="font-mono bg-white/50 dark:bg-transparent border-slate-500/20 dark:border-white/10 focus:border-slate-500 focus:ring-slate-500/50"
                          aria-label={t.githubRepoUrl}
                          placeholder="https://github.com/username/repo"
                          icon={<LinkIcon className="h-4 w-4 text-slate-500 dark:text-slate-400" />}
                      />
                  </div>
                  {url && !isValid && (
                      <p className="text-xs text-red-600 dark:text-red-400 mt-2">Invalid GitHub Repository URL format.</p>
                  )}
                  {fetchError && (
                      <p className="text-xs text-red-600 dark:text-red-400 mt-2">{fetchError}</p>
                  )}
              </div>
          ...
          <div className="flex flex-col h-full max-h-[60vh]">
            <div className="mb-4 flex justify-between items-center gap-4">
              <Input
                ...
                className="flex-1 font-mono bg-white/50 dark:bg-transparent border-teal-500/20 dark:border-white/10 focus:border-teal-500 focus:ring-teal-500/50"
              />
              <Button
                variant="secondary"
                size="sm"
                type="button"
                onClick={toggleAll}
                className="whitespace-nowrap bg-white/50 dark:bg-transparent border-teal-500/20 dark:border-white/10 hover:bg-white/80 dark:hover:bg-white/5 text-teal-800 dark:text-gray-300"
              >
                {selectedFiles.size === files.length ? "Deselect All" : "Select All"}
              </Button>
            </div>
            
            <div className="flex-1 overflow-y-auto custom-scrollbar border border-teal-500/20 dark:border-white/10 rounded-md bg-white/40 dark:bg-black/20 p-2 mb-4 shadow-inner dark:shadow-none">
              {fileTree.length === 0 ? (
                <p className="text-sm text-teal-600/70 dark:text-gray-400 text-center py-4">No files found.</p>
              ) : (
            ...
            <div className="flex justify-between items-center mt-auto pt-2 border-t border-teal-500/20 dark:border-white/10">
              <span className="text-xs text-teal-700/70 dark:text-gray-400">
