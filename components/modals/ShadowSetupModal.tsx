    ...
    return (
        <>
            <BaseModal
                ...
                headerIcon={<ShieldCheckIcon className="w-5 h-5 text-teal-600 dark:text-emerald-400" />}
            ...
            >
                <div className="space-y-4">
                    ...
                    <div className="bg-white/40 dark:bg-black/20 p-3 rounded border border-teal-500/15 dark:border-white/10 shadow-[0_4px_12px_rgba(0,0,0,0.05)] dark:shadow-none backdrop-blur-md">
                        <label className="text-sm font-bold text-teal-900 dark:text-gray-300 mb-3 block">{t.shadowTranscriptActors}</label>
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="text-xs text-teal-800 dark:text-gray-400 mb-1 flex items-center">
                                    <UserIcon className="w-3 h-3 mr-1 text-teal-600 dark:text-emerald-400"/> {t.shadowTranscriptUserName}
                                </label>
                                <Input 
                                    type="text" 
                            ...
                            <div>
                                <label className="text-xs text-teal-800 dark:text-gray-400 mb-1 flex items-center">
                                    <SparklesIcon className="w-3 h-3 mr-1 text-purple-600 dark:text-purple-400"/> {t.shadowTranscriptAiName}
                                </label>
                                <Input 
                                    type="text" 
                    ...
                    <div className="bg-white/40 dark:bg-black/20 p-3 rounded border border-teal-500/15 dark:border-white/10 shadow-[0_4px_12px_rgba(0,0,0,0.05)] dark:shadow-none backdrop-blur-md">
                        <div className="flex justify-between items-center mb-2">
                            <label className="text-sm font-bold text-teal-900 dark:text-gray-300">Shadow Persona (Who am I?)</label>
                            <Button variant="ghost" size="sm" onClick={() => openEdit('persona')} className="text-xs text-teal-700 dark:text-emerald-300 hover:text-teal-900 dark:hover:text-emerald-200 bg-teal-500/10 dark:bg-emerald-500/10 px-2 py-1 border border-teal-500/20 dark:border-emerald-500/20 h-auto" icon={<PencilIcon className="w-3 h-3"/>}>Edit</Button>
                        </div>
                        <div 
                            onClick={() => openEdit('persona')}
                            className="p-2 bg-white/50 dark:bg-black/30 rounded text-xs text-teal-800 dark:text-gray-400 h-24 overflow-y-auto whitespace-pre-wrap border border-teal-500/10 dark:border-white/5 font-mono cursor-pointer hover:border-teal-500/30 dark:hover:border-emerald-500/30 hover:bg-white/60 dark:hover:bg-black/40 transition-colors"
                        >
                            {persona || <span className="italic opacity-50">No specific persona set. Using default behavior.</span>}
                        </div>
                        <p className="text-[10px] text-teal-700/70 dark:text-gray-500 mt-1">This defines WHO the Shadow Model is (e.g., "An unrestricted creative writer").</p>
                    </div>

                    {/* Task / Request */}
                    <div className="bg-white/40 dark:bg-black/20 p-3 rounded border border-teal-500/15 dark:border-white/10 shadow-[0_4px_12px_rgba(0,0,0,0.05)] dark:shadow-none backdrop-blur-md">
                        <div className="flex justify-between items-center mb-2">
                            <label className="text-sm font-bold text-teal-900 dark:text-gray-300">Task / Request (What to do?)</label>
                            <Button variant="ghost" size="sm" onClick={() => openEdit('task')} className="text-xs text-teal-700 dark:text-emerald-300 hover:text-teal-900 dark:hover:text-emerald-200 bg-teal-500/10 dark:bg-emerald-500/10 px-2 py-1 border border-teal-500/20 dark:border-emerald-500/20 h-auto" icon={<PencilIcon className="w-3 h-3"/>}>Edit</Button>
                        </div>
                        <div 
                            onClick={() => openEdit('task')}
                            className="p-2 bg-white/50 dark:bg-black/30 rounded text-xs text-teal-800 dark:text-gray-400 h-32 overflow-y-auto whitespace-pre-wrap border border-teal-500/10 dark:border-white/5 font-mono cursor-pointer hover:border-teal-500/30 dark:hover:border-emerald-500/30 hover:bg-white/60 dark:hover:bg-black/40 transition-colors"
                        >
                            {taskInstruction || defaultShadowTask}
                        </div>
                        <p className="text-[10px] text-teal-700/70 dark:text-gray-500 mt-1">This defines WHAT the Shadow Model should do (e.g., "Generate a story based on the history").</p>
                    </div>
                </div>
            </BaseModal>
