...
export const CacheManagerModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={closeCacheManagerModal}>
      <div className="bg-zinc-900 border border-white/10 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col animate-modal-open" onClick={(e) => e.stopPropagation()}>
            {/* Header */}
            ...
            <div className="p-6 space-y-6 overflow-y-auto max-h-[70vh]">
              ...
              {currentCache && (
                <div className={`p-3 rounded-xl flex items-start gap-3 text-sm ${
                  isInvalid ? 'bg-red-500/10 text-red-400 border border-red-500/20' :
                  isExpired ? 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20' :
              ...
              {/* Info */}
              <div className="bg-zinc-800/50 rounded-xl p-4 text-sm text-zinc-300 space-y-2 border border-white/5">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-end">
                  <label className="text-sm font-medium text-zinc-200">Messages to Cache</label>
                  <span className="text-xs text-zinc-400 font-mono">{messageCount} / {maxMessages}</span>
                </div>
                ...
                <div className="flex justify-between text-xs">
                  <span className={`font-mono ${isEligible ? 'text-emerald-400' : 'text-zinc-500'}`}>
                    ~{estimatedTokens.toLocaleString()} tokens
                  </span>
                  {!isEligible && messageCount > 0 && (
                    <span className="text-red-400">Need 32k+ tokens</span>
                  )}
                </div>
              </div>
            ...
            {/* Footer */}
            <div className="p-4 border-t border-white/10 bg-zinc-800/50 flex justify-between gap-3">
              {currentCache ? (
                <Button
                  variant="ghost"
