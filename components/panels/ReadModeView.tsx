...
const CodeBlock: React.FC<React.PropsWithChildren<{ inline?: boolean; className?: string }>> = memo(({
  ...
  return (
    <div className="relative group/codeblock my-2 rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-zinc-900">
      <div className="sticky top-0 z-10 flex justify-between items-center px-4 py-1.5 bg-gray-100 dark:bg-black/40 rounded-t-xl border-b border-gray-200 dark:border-white/5">
        <span className="text-xs text-gray-500 dark:text-gray-300 font-mono">{lang || 'code'}</span>
        <div className="flex items-center space-x-2"> 
          {isMermaid && (
            <Button
              ...
              className="p-1.5 bg-gray-200 dark:bg-black/30 rounded-md opacity-0 group-hover/codeblock:opacity-100 focus:opacity-100 hover:bg-gray-300 dark:hover:bg-white/10"
            >
              <SitemapIcon className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
            </Button>
          )}
          <Button variant="ghost" onClick={handleCopyCode} title={isCodeCopied ? "Copied!" : "Copy code"} aria-label={isCodeCopied ? "Copied code to clipboard" : "Copy code to clipboard"} className="p-1.5 bg-gray-200 dark:bg-black/30 rounded-md opacity-0 group-hover/codeblock:opacity-100 focus:opacity-100 hover:bg-gray-300 dark:hover:bg-white/10">
              {isCodeCopied ? <CheckIcon className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> : <ClipboardIcon className="w-4 h-4" />}
          </Button>
        </div>
      </div>
      <div className="overflow-hidden rounded-b-xl">
        {lang ? (
            ...
            <pre className="bg-transparent text-gray-900 dark:text-gray-100 p-4 text-sm font-mono whitespace-pre-wrap break-words m-0">
...
const ReadModeView: React.FC<ReadModeViewProps> = memo(({ 
  ...
  return (
    <div
      className="fixed inset-0 bg-white/90 dark:bg-black/90 backdrop-blur-sm z-40 flex flex-col p-4 sm:p-8 md:p-12 pt-24 transition-colors duration-300"
      onClick={onClose} 
      role="dialog"
      aria-modal="true"
    >
      <div className="absolute top-4 right-4 z-10 flex items-center space-x-3">
          ...
          {handleReadModePrev && handleReadModeNext && (
              <div className="flex items-center bg-gray-100 dark:bg-white/5 rounded-full p-1 mr-2 border border-gray-200 dark:border-white/10" dir="ltr">
                  <Button
                      variant="ghost"
      ...
      <div
        ...
        onClick={(e) => e.stopPropagation()} 
      >
        <div className={`bg-white dark:bg-zinc-900 text-gray-900 dark:text-gray-100 border border-gray-200 dark:border-white/10 p-6 sm:p-8 rounded-xl markdown-content shadow-2xl ${getFontSizeClass(readModeFontSizeLevel)} transition duration-200`}>
             <ReactMarkdown 
