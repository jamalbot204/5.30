...
const LogEntryComponent: React.FC<LogEntryProps> = ({ log }) => {
  ...
  if (isMemoryManager) {
      ...
      if (log.requestType === 'chat.create') badgeColor = 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300';
  ...
  return (
    <div className={`border-b ${borderColor} ${containerBg}`}>
      ...
      {isExpanded && (
        <div id={`log-payload-${log.id}`} className="p-3 bg-gray-50 dark:bg-black/30">
          ...
          <pre className={`text-xs whitespace-pre-wrap break-all bg-gray-100 dark:bg-black/20 p-2 rounded-md max-h-96 overflow-auto ${isMemoryManager ? 'text-cyan-600 dark:text-cyan-100 border border-cyan-900/30' : (isToolTrace ? 'text-amber-600 dark:text-amber-100 border border-amber-900/30' : 'text-gray-700 dark:text-gray-300')}`}>
...
const DebugTerminalPanel: React.FC = memo(() => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex justify-center items-center p-4" onClick={closeDebugTerminal}>
      <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-white/10 p-0 rounded-xl shadow-2xl w-full sm:max-w-2xl max-h-[90vh] flex flex-col text-gray-900 dark:text-gray-100 animate-modal-open" onClick={(e) => e.stopPropagation()}>
        <header className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-white/10 sticky top-0 bg-gray-50/80 dark:bg-zinc-900/80 backdrop-blur-md z-10 rounded-t-xl">
          <div className="flex items-center">
            <BugAntIcon className="w-5 h-5 mr-2 text-orange-500 dark:text-orange-400" />
            <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">{t.apiRequestLog}</h2>
          </div>
          <div className="flex items-center space-x-2">
            <Button
              ...
              className="p-1.5 rounded-md hover:text-red-500 dark:hover:text-red-400"
            >
        ...
        <div className="px-4 pt-2 pb-1">
            <p className="text-xs text-gray-500 dark:text-gray-400">{t.showingLogsFor} <span className="font-medium text-gray-700 dark:text-gray-300">{currentChatSession.title}</span></p>
        </div>

        <div className={`flex-1 min-h-0 overflow-y-auto border-t border-gray-200 dark:border-white/10 ${areButtonsDisabled ? 'pointer-events-none opacity-60' : ''}`}>
          {logs.length === 0 ? (
            <p className="p-6 text-center text-gray-400 dark:text-gray-500 italic">{t.noLogsYet}</p>
          ) : (
