...
const AudioTuner: React.FC<AudioTunerProps> = memo(({ triggerRef, onClose }) => {
  ...
  return createPortal(
    <div ref={panelRef} style={style} className="w-64 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-white/10 rounded-lg p-4 shadow-xl z-50 animate-fade-in text-left">
      <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-200 dark:border-white/10">
        <h3 className="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider flex items-center">
            <AdjustmentsHorizontalIcon className="w-3 h-3 mr-1.5 text-emerald-500" />
            Audio Tuner
        </h3>
        <Button variant="ghost" onClick={onClose} className="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-white/10" title="Close"><ChevronDownIcon className="w-3 h-3" /></Button>
      </div>
      ...
      <div className="space-y-5">
        ...
        <div>
          <div className="flex justify-between text-xs mb-1.5">
            <span className="text-gray-700 dark:text-gray-200 font-medium">Playback Speed</span>
            <span className="text-emerald-600 dark:text-emerald-400 font-mono bg-emerald-100 dark:bg-emerald-500/10 px-1.5 rounded border border-emerald-200 dark:border-emerald-500/20">{playbackRate.toFixed(2)}x</span>
          </div>
          <input
            type="range"
            min="0.25"
            ...
            className="w-full h-1.5 bg-gray-200 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer accent-emerald-500 hover:accent-emerald-400"
          />
        ...
        <Button 
            ...
            className="w-full flex items-center justify-between text-xs font-medium py-2 border-t border-white/10 group rounded-none"
        >
