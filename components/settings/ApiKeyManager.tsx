...
const ApiKeyItem: React.FC<{
  ...
  return (
    <div
      ...
      className={`relative p-3 mb-3 rounded-e-xl rounded-s-md border border-white/10 border-s-4 border-s-yellow-500 bg-gradient-to-r from-yellow-500/5 to-transparent flex flex-col sm:flex-row items-center gap-3 cursor-move group transition hover:bg-white/5 ${isFirst ? 'ring-1 ring-yellow-500/30' : ''}`}
    >
      ...
      <div className="flex items-center space-x-1 self-end sm:self-center">
        <Button 
            ...
            variant="ghost"
            size="none"
            className="p-2 text-gray-400 hover:text-yellow-400 bg-white/5 hover:bg-yellow-500/10 rounded-md disabled:opacity-30 disabled:cursor-not-allowed transition-colors h-auto w-auto"
            onMouseDown={(e) => e.stopPropagation()}
        ...
        <Button 
            ...
            variant="ghost"
            size="none"
            className="p-2 text-gray-400 hover:text-red-400 bg-white/5 hover:bg-red-500/10 rounded-md transition-colors h-auto w-auto"
            onMouseDown={(e) => e.stopPropagation()}
...
const ApiKeyManager: React.FC = memo(() => {
  ...
  return (
    <div ref={containerRef} onDragOver={handleDragOverContainer} className="flex flex-col h-full">
      ...
      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-white/10 flex flex-wrap gap-3">
        ...
        <Button onClick={toggleKeyVisibility} title={isKeyVisible ? "Hide Keys" : "Show Keys"} variant="outline" size="none" className="p-2 text-gray-300 bg-white/5 rounded-md hover:text-white border border-white/10 hover:bg-white/10 h-auto w-auto" icon={isKeyVisible ? <EyeOffIcon className="w-5 h-5" /> : <EyeIcon className="w-5 h-5" />} />
        <Button 
