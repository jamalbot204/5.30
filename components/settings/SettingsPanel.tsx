...
const SettingsPanel: React.FC = memo(() => {
    ...
    return (
        <>
            <div className="fixed inset-0 bg-black/80 z-40 flex justify-center items-center p-4" onClick={closeSettingsPanel}>
                <div 
                    className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-white/10 p-0 rounded-2xl shadow-2xl w-full sm:max-w-lg max-h-[90vh] flex flex-col text-gray-900 dark:text-gray-200 relative overflow-hidden min-w-0 animate-modal-open" 
                    onClick={(e) => e.stopPropagation()}
                >
                    {/* Header */}
                    <div className="p-5 pb-0 flex justify-between items-center bg-gray-50 dark:bg-zinc-800/50">
                        <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">{t.settings}</h2>
                        <Button onClick={closeSettingsPanel} variant="ghost" size="none" className="text-gray-500 dark:text-gray-400 p-1.5 rounded-full hover:bg-gray-200 dark:hover:bg-white/10 hover:text-gray-900 dark:hover:text-white transition-colors h-auto w-auto" aria-label={t.close} icon={<CloseIcon className="w-6 h-6" />} />
                    </div>
                    
                    {/* Tabs Navigation */}
                    <div className="flex-shrink-0 flex items-stretch overflow-x-auto overflow-y-hidden custom-scrollbar border-b border-gray-200 dark:border-white/10 px-2 sm:px-5 bg-gray-50 dark:bg-zinc-800/50 min-h-[48px]">
                        {[
                    ...
                    {/* Fixed Footer */}
                    <div className="p-4 border-t border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-zinc-800/80 flex flex-col sm:flex-row justify-between items-center space-y-3 sm:space-y-0">
                        <div className="flex space-x-2 w-full justify-between sm:justify-start">
                            <Button onClick={resetToDefaults} variant="ghost" size="sm" className="px-3 py-2 text-xs font-medium text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors flex items-center h-auto">
                                <ArrowPathIcon className="w-3.5 h-3.5 me-1" />
                                {t.resetDefaults}
                            </Button>
                            <Button onClick={handleMakeDefaults} variant="ghost" size="sm" className="px-3 py-2 text-xs font-medium text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 transition-colors h-auto">
